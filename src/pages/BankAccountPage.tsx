...
import {
  ...
  Link2,
  Mountain,
  Factory, // Hata düzeltildi: 'Industry' yerine 'Factory' kullanıldı
  Tractor,
  AlertTriangle,
  PlusCircle,
  XCircle,
...
const getDefaultState = (): { reportData: ReportData; analysisItems: AnalysisCategory[] } => ({
  ...
  analysisItems: [
    ...
    { id: 'cat2', title: 'Komşu Yapılanma ve Sanayi', icon: Factory, items: [] }, // Hata düzeltildi
...
const RiskDonutChart = React.memo(({ data }: { data: { [key in RiskLevel]: number } }) => {
  ...
  if (total === 0) {
    return <div className="flex items-center justify-center h-full text-gray-400">Analiz Verisi Girilmedi</div>;
  }
  const radius = 60;
  const strokeWidth = 15;
  const circumference = 2 * Math.PI * radius;
  const segments = [
    { value: data.high, color: '#ef4444' },
    { value: data.medium, color: '#f59e0b' },
    { value: data.low, color: '#22c55e' },
    { value: data.opportunity, color: '#3b82f6' }
  ];
...
const LocationAnalysisDashboard = () => {
  ...
  const [reportData, setReportData] = useState<ReportData>(() => {
    try {
      ...
      return getDefaultState().reportData;
    }
  });
  ...
  const [analysisItems, setAnalysisItems] = useState<AnalysisCategory[]>(() => {
    try {
      ...
      return getDefaultState().analysisItems;
    }
  });
  ...
  const removeCategory = useCallback((catId: string) => {
    if (window.confirm("Bu kategoriyi ve içindeki tüm maddeleri silmek istediğinizden emin misiniz?")) {
  ...
  const resetState = useCallback(() => {
    if (window.confirm("Tüm verileriniz silinecek ve uygulama başlangıç durumuna dönecek. Emin misiniz?")) {
    ...
    <div className="flex h-screen bg-gray-100 font-sans">
        ...
        <div ref={reportRef} className="bg-white max-w-4xl mx-auto p-12 shadow-2xl rounded-lg border border-gray-200">
          ...
          <section className="mb-10">
            ...
            <div className="bg-gray-50 p-6 rounded-lg border">
              <RiskDonutChart data={riskCounts} />
            </div>
          </section>
          <section className="mb-10">
            <h2 className="text-2xl font-semibold text-gray-700 border-b pb-2 mb-6">Detaylı Analiz</h2>
            <div className="space-y-6">
              {analysisItems.map(cat => cat.items.length > 0 && (
                <div key={cat.id}>
                  <h3 className="flex items-center text-xl font-semibold text-gray-600 mb-3"><cat.icon size={20} className="mr-3 text-gray-400" /> {cat.title}</h3>
          ...
          <footer className="text-center text-xs text-gray-400 border-t pt-4 mt-12">
            PestMentor Konum Analiz Raporu - {new Date().getFullYear()}
          </footer>
        </div>
      </main>
      ...
      <aside className="w-[450px] bg-gray-50 border-l border-gray-200 p-6 flex flex-col">
        ...
        <div className="space-y-4 overflow-y-auto flex-1 pr-2">
          <AccordionItem title="Genel Rapor Bilgileri">
          ...
          <AccordionItem title="Analiz Kategorileri ve Maddeleri">
            <div className="space-y-4">
              {analysisItems.map(cat => (
                <div key={cat.id} className="bg-gray-100 p-3 rounded-lg border">
                  <div className="flex justify-between items-center mb-2">
                    {editingCategoryId === cat.id ?
                      (<input type="text" value={editingCategoryTitle} onChange={e => setEditingCategoryTitle(e.target.value)} className="font-medium text-sm text-gray-700 bg-white border-blue-500 border rounded px-1" autoFocus onBlur={() => saveCategoryTitle(cat.id)} onKeyDown={(e) => e.key === 'Enter' && saveCategoryTitle(cat.id)}/>) :
                      (<h4 className="font-medium text-sm text-gray-700">{cat.title}</h4>)
                    }
                    <div className="flex items-center space-x-2">
                      {editingCategoryId === cat.id ?
                        (<button onClick={() => saveCategoryTitle(cat.id)} className="text-green-600 hover:text-green-800"><Save size={16} /></button>) :
                        (<button onClick={() => startEditingCategory(cat)} className="text-blue-600 hover:text-blue-800"><Edit size={16} /></button>)
                      }
                      ...
                      <div key={item.id} className="bg-white p-2 rounded border">
                        ...
                        <div className="flex space-x-1 mt-2">
                          {Object.keys(riskColors).map(risk => (
                            <button key={risk} onClick={() => handleRiskChange(cat.id, item.id, risk as RiskLevel)} className={`px-1.5 py-0.5 text-xs rounded transition-all ${item.risk === risk ? `${{low:'bg-green-500',medium:'bg-amber-500',high:'bg-red-500',opportunity:'bg-blue-500'}[risk as RiskLevel]} text-white` : 'bg-gray-200 hover:bg-gray-300'}`}>
                              {risk === 'opportunity' ? <Zap size={12} /> : risk[0].toUpperCase()}
                            </button>
                          ))}
                        </div>
              ...
              <button onClick={addCategory} className="w-full mt-2 p-2 text-sm text-blue-600 bg-blue-50 hover:bg-blue-100 rounded-lg border border-dashed border-blue-300 transition-colors">
                Yeni Kategori Ekle
              </button>
            </div>
          </AccordionItem>
          <AccordionItem title="Stratejik Özet">
        ...
        <div className="mt-6 pt-6 border-t">
          ...
          <div className="flex space-x-2">
            <button onClick={() => generateReport('pdf')} disabled={loading} className="w-full flex items-center justify-center bg-blue-600 text-white px-4 py-2.5 rounded-md hover:bg-blue-700 disabled:bg-gray-400 transition-all">
